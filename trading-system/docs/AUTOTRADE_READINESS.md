...
| F10 | High     | **Walk-forward windows blended eras.** Windows were sliced by row position, so with mixed-history universes (SOL lists 2020, BTC 2017) a window labeled 2017–2019 silently included later-era rows of short-history symbols. The full-sample grid metrics and the PASS gate were **not** affected (they align by date); only the per-window walk-forward table was contaminated. | **Fixed** (2026-07-06): windows slice by date on the benchmark calendar; symbols without data in a window are absent from it. Re-run `run_backtest.py --config h1b` for corrected walk-forward numbers. |
